...
- **State Discovery:** At the start of every session (or when `/sync` is called), you MUST:
    ...
    3. Read `HANDOFF.md` to recover the narrative context from the last agent.
       - For deep historical context (sessions older than the 10 most recent), read `HANDOFF-ARCHIVE.md`.
...
2. **Update `HANDOFF.md`** (Two-File System):
   - **Two-File Structure**:
     - `HANDOFF.md` - Active file with the 10 most recent sessions
     - `HANDOFF-ARCHIVE.md` - Historical archive with older sessions
   - **INSERT** a new session entry at the TOP of HANDOFF.md (below the archive link) with:
   ...
   - **Archive Rotation**: After adding a new session, if HANDOFF.md has more than 10 sessions:
     1. Identify the oldest session (last `## Session` before shared sections)
     2. Remove it from HANDOFF.md
     3. Insert it at the TOP of HANDOFF-ARCHIVE.md (after the archive header)
...
Available commands (full documentation in `.claude/commands/`):
- `/sync` - Project state synchronization (reads features.json, HANDOFF.md, verifies environment)
- `/commit` - Semantic git commits with conventional format
- `/ship` - Feature shipping workflow (updates features.json, HANDOFF.md, commits)
- `/add` - Create new custom slash commands